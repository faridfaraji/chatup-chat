...
def initiate_conversation(customer: dict):
    ...
    if conv_id:
        ...
        conv_id = db_client.add_conversation(shop_id)
    CONVERSATIONS[conv_id] = {"conversation_chain": create_conversation_chain(), "shop_id": shop_id}
    return conv_id
...
def respond_customer_message(conversation_id: str, message: str):
    ...
    if conversation_id in CONVERSATIONS:
        ...
        if conv_chain is None:
            CONVERSATIONS[conversation_id]["conversation_chain"] = create_conversation_chain()
